...
void destroy_quantizer (quantizer_t *q)
{
	int i;
...
int get_color_index (quantizer_t *q, uint32_t color)
{
	hexnode_t *f, *l;
...
void insert_color (quantizer_t *q, uint32_t color)
{
	uint8_t *v = (uint8_t *)&color;
	hexnode_t *n = q->root;
	hexnode_t *f, *l;
...
int get_palette (quantizer_t *q, uint32_t pal[COLORS + 1])
{
	int index;
